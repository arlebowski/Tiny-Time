...
const AnalyticsTab = ({ user, kidId, familyId }) => {
  ...
  const [activeModal, setActiveModal] = useState(null); // 'activity' | 'feeding' | 'sleep' | null
  ...
  // ---------- Apple-ish Card + Modal helpers (Phase 2: structure only) ----------
  const AppleCard = ({ title, subtitle, rightText, children, onClick }) =>
    React.createElement(
      'button',
      {
        type: 'button',
        onClick,
        className:
          'w-full bg-white rounded-2xl shadow-lg p-5 text-left active:scale-[0.99] transition',
      ...
      React.createElement(
        ...
        React.createElement(
          'div',
          null,
          React.createElement('div', { className: 'text-[15px] font-semibold text-gray-900' }, title),
          subtitle
            ? React.createElement('div', { className: 'mt-1 text-[13px] text-gray-500' }, subtitle)
            : null
        ),
        rightText
          ? React.createElement('div', { className: 'text-[13px] font-medium text-gray-400' }, rightText)
          : null
      ),
      children
    );
  ...
  const FullscreenModal = ({ title, onClose, children }) =>
    React.createElement(
      ...
      React.createElement(
        ...
        {
          className:
            'sticky top-0 z-10 bg-white/90 backdrop-blur border-b border-gray-100'
        },
        React.createElement(
          'div',
          ...
          React.createElement(
            'button',
            {
              ...
              onClick: onClose,
              className:
                'px-3 py-1.5 rounded-lg text-[14px] font-medium text-indigo-600 hover:bg-indigo-50 active:bg-indigo-100'
            },
            'Done'
          )
        )
      ...
      React.createElement(
        'div',
        { className: 'px-4 py-4 overflow-y-auto', style: { height: 'calc(100vh - 52px)' } },
        children
      )
    );
  ...
  // --------- Build EXISTING sections as variables so we can reuse in cards + details ----------

  const activitySection = React.createElement(
    React.Fragment,
    null,
    React.createElement(
      'div',
      { className: 'mt-2 mb-2' },
      React.createElement('div', { className: 'text-[18px] font-semibold text-gray-900' }, 'Daily Activity')
    ),
    React.createElement(DailyActivityChart, {
      viewMode: timeframe,
  ...
  const feedingSection = React.createElement(
    React.Fragment,
    null,
    React.createElement(
      'div',
      { className: 'mt-4 mb-2' },
      React.createElement('div', { className: 'text-[18px] font-semibold text-gray-900' }, 'Feeding stats')
    ),
    // (Existing Feeding cards + charts remain exactly as they are below)
    React.createElement(
      'div',
      ...
      [
        ...
        React.createElement(
          ...
          React.createElement(
            'div',
            { className: 'text-3xl font-bold text-indigo-600 leading-none' },
            stat.value,
    ...
    React.createElement(
      ...
      React.createElement(
        ...
        React.createElement('div', { className: 'text-3xl font-bold text-indigo-600 leading-none' }, stats.avgFeedingsPerDay.toFixed(1)),
        React.createElement('div', { className: 'text-xs text-gray-400 mt-1' }, stats.labelText)
      ),
      React.createElement(
        'div',
        ...
        React.createElement('div', { className: 'text-3xl font-bold text-indigo-600 leading-none' }, formatInterval(stats.avgInterval)),
        React.createElement('div', { className: 'text-xs text-gray-400 mt-1' }, stats.labelText)
      )
    ),
    React.createElement(
  ...
  const sleepSection = React.createElement(
    React.Fragment,
    null,
    React.createElement('div', { className: 'mt-4 mb-2' },
      React.createElement('div', { className: 'text-[18px] font-semibold text-gray-900' }, 'Sleep stats')
    ),
    React.createElement(
      "div",
      { className: "grid grid-cols-2 gap-4" },
      React.createElement(
        ...
        React.createElement(
          "div",
          { className: "flex items-baseline justify-start gap-1 text-3xl font-bold text-indigo-600 leading-none" },
          Number(sleepCards.avgTotal || 0).toFixed(1),
    ...
    React.createElement(
      "div",
      { className: "bg-white rounded-2xl shadow-lg p-6" },
      React.createElement("div", { className: "text-sm font-medium text-gray-600 mb-2.5 text-center" }, "Sleep history"),
      sleepBuckets.length > 0
        ? React.createElement(
            "div",
            { className: "relative" },
  ...
  return React.createElement(
    ...
    // ---------------------------
    // PHASE 2: Highlights (Apple-style cards)
    // ---------------------------
    React.createElement('div', { className: 'mt-2 space-y-4' },
      React.createElement(AppleCard, {
        title: 'Daily Activity',
        subtitle: 'Timing and rhythm across the day.',
        rightText: 'View',
        onClick: () => setActiveModal('activity')
      },
        React.createElement('div', { className: 'mt-4' },
          React.createElement('div', { className: 'text-[12px] text-gray-500 mb-2' }, 'Tap to expand'),
          // lightweight preview: reuse the existing actogram card (keeps parity)
          React.createElement('div', { className: 'pointer-events-none' }, activitySection)
        )
      ),

      React.createElement(AppleCard, {
        title: 'Feeding',
        subtitle: `Avg ${Number(stats.avgVolumePerDay || 0).toFixed(1)} oz/day · ${stats.labelText || ''}`.trim(),
        rightText: 'View',
        onClick: () => setActiveModal('feeding')
      },
        React.createElement('div', { className: 'mt-4 text-[13px] text-gray-600' },
          `Avg ${Number(stats.avgVolumePerFeed || 0).toFixed(1)} oz/feed · ${Number(stats.avgFeedingsPerDay || 0).toFixed(1)} feeds/day`
        )
      ),

      React.createElement(AppleCard, {
        title: 'Sleep',
        subtitle: `Avg ${Number(sleepCards.avgTotal || 0).toFixed(1)} hrs/day · ${sleepCards.label || ''}`.trim(),
        rightText: 'View',
        onClick: () => setActiveModal('sleep')
      },
        React.createElement('div', { className: 'mt-4 text-[13px] text-gray-600' },
          `Day ${Number(sleepCards.avgDay || 0).toFixed(1)} · Night ${Number(sleepCards.avgNight || 0).toFixed(1)}`
        )
      )
    ),

    // ---------------------------
    // PHASE 2: Details (keep everything, just collapsed by default)
    // ---------------------------
    React.createElement('div', { className: 'mt-6 space-y-4' },
      React.createElement(
        ...
        {
          type: 'button',
          onClick: () => setShowFeedingDetails(v => !v),
          className: 'w-full flex items-center justify-between text-[14px] font-semibold text-gray-800'
        },
        'Feeding details',
        React.createElement('span', { className: 'text-gray-400 font-medium' }, showFeedingDetails ? 'Hide' : 'Show')
      ),
      showFeedingDetails ? feedingSection : null,
      ...
      React.createElement(
        'button',
        {
          ...
          className: 'w-full flex items-center justify-between text-[14px] font-semibold text-gray-800 mt-2'
        },
        'Sleep details',
        React.createElement('span', { className: 'text-gray-400 font-medium' }, showSleepDetails ? 'Hide' : 'Show')
      ),
      showSleepDetails ? sleepSection : null
    ),

    // ---------------------------
    // Full-screen modals (reuse existing sections; nothing is reimplemented)
    // ---------------------------
    (activeModal === 'activity'
      ? React.createElement(FullscreenModal, { title: 'Daily Activity', onClose: () => setActiveModal(null) }, activitySection)
      : null),
    (activeModal === 'feeding'
      ? React.createElement(FullscreenModal, { title: 'Feeding', onClose: () => setActiveModal(null) }, feedingSection)
      : null),
    (activeModal === 'sleep'
      ? React.createElement(FullscreenModal, { title: 'Sleep', onClose: () => setActiveModal(null) }, sleepSection)
      : null)
  );
};
