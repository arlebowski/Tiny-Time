...
test('cadence is every 5 logs starting at 5', () => {
  assert.equal(LOG_INTERVAL, 5);
  assert.equal(isCadenceLog(0), false);
  assert.equal(isCadenceLog(4), false);
  assert.equal(isCadenceLog(5), true);
  assert.equal(isCadenceLog(6), false);
  assert.equal(isCadenceLog(10), true);
  assert.equal(isCadenceLog(15), true);
});

test('first cadence log is eligible with empty state', () => {
  const now = new Date(2026, 8, 5, 16, 0).getTime();
  const state = { ...EMPTY_STATE, logCount: 5 };
  assert.equal(isEligibleToShow(state, now), true);
});

test('non-cadence logs are never eligible even with no caps', () => {
  const now = Date.now();
  assert.equal(isEligibleToShow({ ...EMPTY_STATE, logCount: 6 }, now), false);
});
...
test('applyShown increments the local-day counter', () => {
  const now = new Date(2026, 8, 5, 16, 0).getTime();
  const next = applyShown({ ...EMPTY_STATE, logCount: 5 }, now);
...
test('withCurrentDay resets showsOnDay across midnight', () => {
  ...
  const rolled = withCurrentDay(
    { ...EMPTY_STATE, dayKey: '2026-09-05', showsOnDay: 3, logCount: 10 },
    now
  );
