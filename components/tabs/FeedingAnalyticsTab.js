const FeedingAnalyticsTab = ({ user, kidId, familyId, setActiveTab }) => {
  ...
  const feedLabelIcon = (window.TT?.shared?.icons?.BottleV2 || window.TT?.shared?.icons?.["bottle-v2"] || window.TT?.shared?.icons?.BottleMain || window.TT?.shared?.icons?.["bottle-main"]) || null;
