...
const HorizontalCalendar = ({ initialDate = new Date(), onDateSelect }) => {
  ...
  return (
    React.createElement('div', {
      className: "w-full text-white font-sans select-none overflow-hidden"
    },
      React.createElement('header', { className: "mb-6 flex items-center justify-between" },
        ...
        React.createElement('div', { className: "flex gap-1" },
          React.createElement('button', {
            ...
            React.createElement(__ttChevronLeft, {
          ...
          React.createElement('button', {
            ...
            className: cn(
              "p-2 hover:bg-white/10 rounded-full transition-colors group",
              weeksOffset === 0 && "opacity-0 pointer-events-none"
            ),
            'data-testid': "button-next"
          },
            React.createElement(__ttChevronRight, {
              className: "w-6 h-6 text-white/40 group-hover:text-white transition-colors"
            })
          )
        )
      ...
      React.createElement('div', { className: "relative touch-none" },
        React.createElement(AnimatePresence, { mode: "wait", custom: direction },
          React.createElement(motion.div, {
            ...
            className: "flex justify-between items-center gap-2 cursor-grab active:cursor-grabbing"
          },
            days.map((date, index) => {
              ...
              return (
                React.createElement(motion.button, {
                  ...
                  className: cn(
                    "relative flex flex-col items-center justify-center flex-1 h-[110px] rounded-2xl transition-colors duration-300 group focus:outline-none shrink-0",
                    isSelected ? "bg-[#333333] shadow-lg shadow-white/5" : "hover:bg-white/5"
                  ),
                  ...
                  React.createElement('span', {
                    className: cn(
                      "text-base font-semibold mb-4 leading-none text-white"
                    )
                  }, format(date, "d")),
