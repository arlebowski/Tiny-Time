const DiaperAnalyticsTab = ({ user, kidId, familyId, setActiveTab }) => {
  ...
  const loadAnalytics = async () => {
    ...
    setLoading(true);
    try {
      const changes = await firestoreStorage.getAllDiaperChanges();
      const safeChanges = Array.isArray(changes) ? changes : [];
      setAllDiaperChanges(safeChanges);
      calculateStats(safeChanges);
    } finally {
      setLoading(false);
    }
  };

  const getChangeTimestamp = (c) => Number(c?.timestamp || 0);

  const calculateStats = (changes) => {
    ...
    const allUniqueDays = new Set(
      changes.map(c => new Date(getChangeTimestamp(c)).toDateString())
    ).size;
    ...
    const recentChanges = changes.filter(
      c => {
        const ts = getChangeTimestamp(c);
        return ts >= periodStart && ts < periodEnd;
      }
    ).sort((a, b) => getChangeTimestamp(a) - getChangeTimestamp(b));

    const totalChanges = recentChanges.length;
    const totalWet = recentChanges.filter(c => !!c.isWet).length;
    const totalPoo = recentChanges.filter(c => !!c.isPoo).length;

    const daysInPeriod = Math.ceil((periodEnd - periodStart) / MS_PER_DAY);
    const avgChangesPerDay =
      daysInPeriod > 0 ? totalChanges / daysInPeriod : 0;
    const avgWetPerDay =
      daysInPeriod > 0 ? totalWet / daysInPeriod : 0;
    const avgPooPerDay =
      daysInPeriod > 0 ? totalPoo / daysInPeriod : 0;

    let totalIntervalMinutes = 0;
    for (let i = 1; i < recentChanges.length; i++) {
      totalIntervalMinutes +=
        (getChangeTimestamp(recentChanges[i]) -
          getChangeTimestamp(recentChanges[i - 1])) /
        (1000 * 60);
    }
    const avgInterval =
      recentChanges.length > 1
        ? totalIntervalMinutes / (recentChanges.length - 1)
        : 0;

    const chartData = generateChartData(changes, timeframe);
  ...
  const generateChartData = (changes, range) => {
    ...
    filtered.forEach(c => {
      const dayStart = new Date(getChangeTimestamp(c));
      dayStart.setHours(0, 0, 0, 0);
      const key = dayStart.getTime();
  ...
  const maxChanges = Math.max(...stats.chartData.map(d => d.changes || 0), 1);
  ...
  return React.createElement(
    ...
    React.createElement(
      ...
      React.createElement(
        ...
        diaperLabelIcon && React.createElement(diaperLabelIcon, {
          className: 'w-5 h-5',
          style: {
            color: 'var(--tt-diaper)',
            strokeWidth: '1.5',
    ...
    React.createElement(
      ...
      React.createElement(
        ...
        [
          { label: 'Changes / Day', value: stats.avgChangesPerDay.toFixed(1) },
          { label: 'Wet / Day', value: stats.avgWetPerDay.toFixed(1) }
        ].map(stat =>
          React.createElement(
            'div',
            {
                        ...
                        React.createElement(
                          'div',
                          {
                            ...
                            style: {
                              ...
                              height: `${(Number(item.changes || 0) / maxChanges) * 160}px`,
                              minHeight: '30px'
                            }
                          },
                          React.createElement(
                            ...
                            React.createElement('span', { className: 'text-xs' }, Number(item.changes || 0))
                          )
                        )
                      ),
                      React.createElement('div', {
...
window.TT.tabs.DiaperAnalyticsTab = DiaperAnalyticsTab;
